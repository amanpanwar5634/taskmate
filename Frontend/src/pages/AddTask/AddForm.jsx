...
export default function AddForm({ onTaskAdded }) {
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        try { const res = await axiosInstance.post("/task/create", TaskInfo, { headers: { id: authUser._id }, });
     if (res.data.taskcreated) {
        toast.success("Task added successfully");
        document.getElementById("my_modal_1").close();
         setTimeout(()=>{window.location.reload();},1000);
         onTaskAdded(res.data.taskcreated);  
         }
        } catch (err) {
    ...
    return (
        ...
        <form method="dialog" onSubmit={handleSubmit(onSubmit)}>
         ...
         document.getElementById("my_modal_1").close()}> ✕ </button>
        </Link>
        ...
        <input type="text" placeholder="Enter title" className="w-80 mt-1 outline-none bg-white-300"
            ...
            <div className="mt-2 px-4 py-1">
                 <h3 className="mb-1">Description</h3>
                    <textarea  placeholder="Enter description"  className="w-80 mt-1 outline-none bg-white-300"
